...
================================================== -->
<meta charset="utf-8">
<title>Sucessful & Unsucessful Candidates</title>
			      ...
			      if($row_seat_list=mysql_fetch_object($result_seat_list)){
				      ...
				      if($row_ledger=mysql_fetch_object($result_ledger)){
				       	$sl_id=$row_ledger->SL_ID;
				                  ...
				                  $sno=0;
				                  $query="SELECT FORMAT(CGPA,2) AS CGPA,OBTAIN_MARKS,RESULT_REMARKS,FORMAT(PERCENTAGE,2) AS PERCENTAGE,ROLL_NO,TOTAL_MARKS FROM ledger_detail_summary WHERE SL_ID=$sl_id  ORDER BY CGPA DESC,PERCENTAGE DESC"; 
						      ...
						      while($row_ledger_detail_summary=mysql_fetch_object($result_ledger_detail_summary)){
								...
								if($sno==0){	
									...
									echo("<TR>");
									echo("    <TH colspan=10  style='text-align:center' class='success'><h3>$program_ann</h3></TH>");
									echo("</TR>");
	                                                echo("<TR class='info'>");
									echo("  	<TH>S.NO.</TH>");
									echo("  	<TH>ROLL NO.</TH>");
								...
								$RESULT_REMARKS=$row_ledger_detail_summary->RESULT_REMARKS;
