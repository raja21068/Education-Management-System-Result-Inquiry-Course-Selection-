<?php 
     $ret =  mysql_select_db('uos_exam_wing',mysql_connect('localhost','root',''));
	      ...
	      while($row=mysql_fetch_object($result)){
				...
				$is_record_available=true;
                        ...
                        $courseNo = $row->COURSE_NO ;
						 $que1 = "SELECT DISTINCT(COURSE_TITLE) FROM ac_scheme_detail WHERE COURSE_NO LIKE '%$courseNo%' AND SCHEME_ID=".$row->SCHEME_ID;
						 $resultSet = mysql_query($que1);
						 $rows=mysql_num_rows($resultSet1)
							if($rows==0){						 
