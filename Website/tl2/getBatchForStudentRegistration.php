		 ...
		 // echo($query);
	      ...
	      while($row_batch=mysql_fetch_object($result_batch)){
		      ...
		      while($row_part=mysql_fetch_object($result_part)){
				  ...
				  $query="select SL.SL_ID, SL.PART,SL.BATCH_ID,SL.PREP_DATE,SL.YEAR,SL.REMARKS,SL.PART_GROUP,SL.TYPE from seat_list SL, ledger 
				  L WHERE SL.BATCH_ID=$batch_id and SL.PART=$part and SL.year='$exam_year' AND SL.SL_ID=L.SL_ID AND SL.PART=L.SCHEME_PART";
				...
				$result_seat_list=mysql_query($query);
			      while($row_seat_list=mysql_fetch_object($result_seat_list)){
					...
					echo("<option value='$SL_ID'>$program_ann</option>");
